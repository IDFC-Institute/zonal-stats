...
Zonal_Stat_NTL <- function(x,y) # Define function
{
  ...
  BQ=paste(substr(names(x),(stri_length(names(x))-4),(stri_length(names(x)))), y[[8]], sep="_")
  ...
  z<-cbind(y[[1]],y[[2]],y[[3]],y[[4]],y[[5]],y[[6]],y[[7]],y[[8]],CQ,R,BQ) # Bind data with shape file
  return(z) 
}
...
NTL<-temp_raster_NTL
proc.time() - ptm
colnames(NTL) <- c("City_Name","State_Name","State_Sensus_Code","City_ID","District_Name","Dist_Census_Code","City_Sharing_Dist","Area","Year","NTL_Data_Source","NTL_Count","NTL_Lit_Up_Pixcel_Count","Lit_up_pixel_Percent","NTL_Mean","NTL_Min","NTL_Max","NTL_Median","NTL_Std_Dev","NTL_Sum","NTL_Mode","Sys_Genrated_UID") # Change column header

write.csv(NTL,"C:/Users/Admin/Desktop/Intercalibrated_NTL/NTL_Admin_ZS.csv", na="NA") # Enter Output csv file name and path 

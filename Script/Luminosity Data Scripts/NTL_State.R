...
Zonal_Stat_NTL <- function(x,y) # Define function
{
  ...
  BQ=paste(substr(names(x),(stri_length(names(x))-4),(stri_length(names(x)))), y[[4]], sep="_")
  ...
  z<-cbind(y[[1]],y[[2]],y[[3]],y[[4]],CQ,R,BQ) # Bind data with shape file
  return(z) 
}
...
NTL<-temp_raster_NTL
proc.time() - ptm
colnames(NTL) <- c("State_Name","State_Sensus_Code","Name_Type","Year","Area","NTL_Data_Source","NTL_Count","NTL_Lit_up_pixel_count","NTL_Lit_up_pixel_percent","NTL_Mean","NTL_Min","NTL_Max","NTL_Median","NTL_Std_Dev","NTL_Sum","NTL_Mode","Unique_ID") # Change column header

write.csv(NTL,"F:/NTL/NTL_State_ZS.csv", na="NA") # Enter Output csv file name and path 
